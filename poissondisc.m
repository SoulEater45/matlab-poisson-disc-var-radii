function [points, pointRadii] = poissondisc(space, radii, varargin)
...
if ismatrix(space) && size(space, 1) == 1
    % number of dimensions of the cube/sphere
    nN = length(space);
else
...
for i = 1:2:numel(varargin)
    if (~isa(varargin{i}, 'char'))
        error('Additional input at position %.0f has to be a char array', i);
    end
    
    switch lower(varargin{i})
        case 'repetitions'
            if ~(isa(varargin{i+1}, 'double') || contains(class(varargin{i+1}), 'int'))
                warning(['Casting "repetitions" argument to double and taking the floor of it.', 10, ...
                    'Please provide an appropriate data type to avoid problems.', 10, ...
                    'Supporting double and integer types.']);
            end
            nK = floor(double(varargin{i+1}));
        case 'additionaloffset'
            if ~isa(varargin{i+1}, 'double')
                warning(['Casting "repetitions" argument to double.', 10, ...
                    'Please provide an appropriate data type to avoid problems.']);
            end
            fO = double(varargin{i+1});
        case 'visualise'
            bVis = logical(varargin{i+1}) && any(nN == [2,3]);
        case 'spherical'
            bSphere = logical(varargin{i+1});
        case 'centered'
            bCentered = logical(varargin{i+1});
        % case 'nonneg'
        %     nNonNeg = logical(varargin{i+1});
        otherwise
            warning('No option %s found', varargin{i});
    end
end
